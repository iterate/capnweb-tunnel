...
- [x] Add a Node weather reporter example and Vitest coverage that starts a Node server in a subprocess, connects `createCaptunTunnel()`, intercepts `wttr.in`, and verifies `/weather?city=...`. _`examples/node/server.ts` plus `examples/node/node.test.ts` cover the Node subprocess._
