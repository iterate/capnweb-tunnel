...
const name = args.includes("--name")
  ...
  : words.map((list) => list[Math.floor(Math.random() * list.length)]).join("-");
const secret = args.includes("--secret")
  ? args[args.indexOf("--secret") + 1]
  : process.env.CAPTUN_SECRET;
const port =
  args.find(
    (arg, index) =>
      !arg.startsWith("--") && args[index - 1] !== "--name" && args[index - 1] !== "--secret",
  ) ?? "3000";
...
console.log(`tunneling ${base.href} -> http://localhost:${port}`);
await new Promise(() => {});
